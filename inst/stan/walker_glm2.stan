...
functions {
  ...
  vector y_original, vector xbeta_fixed) {
    ...
    int k = rows(xreg); // dim of regressors, time points and latent state dims
    int n = rows(y);  // dim time points
    int m = rows(a1);  // dim latent state
    matrix[n, 2] loglik = rep_matrix(negative_infinity(), n, 2); 
    // Final importance sampling correction to log likelihood | [:,1] is Gaussian approximation [:, 2] is correction
    vector[m] x = a1; // initial state estimate D&K 2012 4.12 
    matrix[m, m] P = P1; // initial state variance D&K 2012 eq 4.12
    vector[n] v; // Forecast innovation D&K 2012 eq 4.13
    vector[n] F; // Forecast error variance D&K 2012 Eq 4.16 F_t=Z_t P_T Z_t^' + H_t
    matrix[m, n] K; // Kalman gain
    matrix[m, n+1] r; // Smoother backward pass recursion variable Eq 4.39 in D&K 2012 
    vector[m] tmpr; // temp variable 
